...
-- [EXECUTOR] Module registry replaces script hierarchy
local iconModule = _M
local Reference = _M.Reference
-- Executor always owns the module (no leadPackage redirect)
...
-- MODULES
local Signal = _M.Signal
local Janitor = _M.Janitor
local Utility = _M.Utility
local Themes = _M.Themes
local Gamepad = _M.Gamepad
local Overflow = _M.Overflow
local Icon = {}
Icon.__index = Icon
...
local themes = _M  -- [EXECUTOR] themes accessed via _M
local iconsDict = {}
local anyIconSelected = Signal.new()
local elements = _M  -- [EXECUTOR] elements accessed via _M
...
function Icon.new()
	...
	-- [EXECUTOR] ResetOnSpawn not applicable
